...
class SlickManagedArrayBlockingQueue[E >: Null <: PriorityRunnableProxy](
  maximumInUse: Int,
  capacity: Int,
  fair: Boolean = false
) extends util.AbstractQueue[E] with BlockingQueue[E] with Logging { self =>
  ...
  private[util] def increaseInUseCount(pr: PriorityRunnableProxy): Unit = {
  ...
  private[this] def extract(): E = {
    if (highPrioItemQueue.count != 0) {
      highPrioItemQueue.extract
  ...
  def poll(timeout: Long, unit: TimeUnit): E = {
    ...
    lockedInterruptibly {
      while (counts == 0) {
        if (nanos <= 0) return null //scalastyle:ignore
  ...
  override def remove(o: AnyRef): Boolean = {
    if (o.eq(null)) { //scalastyle:ignore
      false
    } else {
      locked {
